...
document.addEventListener('DOMContentLoaded', () => {
 ...
 // const questions = [ /* … your 40 question objects … */ ];

  // Pause every 6s
  const timestamps = questions.map((_, i) => (i + 1) * 6);

  // Badge JSONs now on CDN
  const badgeFiles = Array.from({ length: 13 }, (_, i) =>
    `https://zencareer.b-cdn.net/badges/${i + 1}.json`
  );
  ...
  lottie.loadAnimation({
    container: finalBadge,
    renderer: 'svg',
    loop: false,
    autoplay: true,
    path: badgeFiles[badgeFiles.length - 1],
  });
  ...
  function showQuestion(i) {
    gsap.to(progressBar, { width: `${(i / questions.length) * 100}%`, duration: 0.5 });
  ...
  function showResults() {
    ...
    resultSummary.textContent =
      'Congrats! You’ve completed your world exploration quiz. Your unique profile is ready.';
  ...
  emailForm.onsubmit = e => {
    e.preventDefault();
    alert('Thanks! We’ll send your full report shortly.');
  };
});
